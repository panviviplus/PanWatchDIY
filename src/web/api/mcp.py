...
from marketdata.vendors.tencent import fetch_raw as _fetch_tencent_raw

# 辅助：将 PanWatch 代码转为腾讯格式
def _tencent_symbol(code: str, market: str) -> str:
    """将股票代码转为腾讯行情查询格式。"""
    try:
        return Symbol(code, Symbol.guess_market(code, market)).to_tencent()
    except Exception:
        # 回退到简单拼接
        if market == "HK":
            return f"r_hk{code}"
        elif market == "US":
            return f"t_us{code.lower()}"
        else:
            return f"sh{code}" if code.startswith(("6", "5")) else f"sz{code}"


def _fetch_tencent_quotes(symbols: list[str]) -> list[dict]:
    """使用 marketdata 包获取腾讯行情数据。"""
    return _fetch_tencent_raw(symbols)
...
def require_mcp_user(
    ...
    realm = "Basic realm=PanWatchDIY-MCP"
    ...
    if not basic_user_pass:
        raise HTTPException(
            ...
            headers={"WWW-Authenticate": "Bearer, Basic realm=PanWatchDIY-MCP"},
        )
...
def _trade_position(arguments: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    try:
        ...
        if not position:
            raise McpToolError(
                error_code=ERR_NOT_FOUND,
                message=f"持仓不存在: id={position_id}",
                hint="请检查 position_id 是否正确。",
            )

        if action == "add":
            # 加仓：计算新的加权成本价
            old_cost = float(position.cost_price or 0)
            ...
            position.invested_amount = (position.invested_amount or 0) + (price * quantity)
            db.commit()
            db.refresh(position)
            result = {
                "position_id": position.id,
            ...
            if quantity > old_qty:
                raise McpToolError(
                    error_code=ERR_INVALID_PARAMS,
                    message=f"减仓数量({quantity})超过持仓({old_qty})",
                    hint="请检查 quantity 参数。",
                )
            new_qty = old_qty - quantity
            old_cost = float(position.cost_price or 0)
            realized = (price - old_cost) * quantity if old_cost > 0 else 0
            ...
            trade = PositionTrade(
                ...
                cost_before=old_cost,
                qty_before=old_qty,
                cost_after=old_cost if new_qty > 0 else None,
                qty_after=new_qty,
                note=note or "",
                traded_at=traded_at or datetime.now(),
            )
            ...
            if new_qty == 0:
                ...
                position.closed_at = traded_at or datetime.now()
